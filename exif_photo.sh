...
check_image_dimensions() {
    ...
    if [ "$width" -lt 300 ] || [ "$height" -lt 300 ]; then
        return 1  # Image is too small
    else
        return 0  # Image is large enough
    fi
}
...
# Array of allowed photo extensions
allowed_extensions=("gif" "heic" "rw2" "dng" "heif" "jpeg" "jpg" "orf" "png" "raw" "tif" "tiff" "nef" "cr2" "cr3" "arw" "raf" "sr2" "srw")
...
if [ "$log_file" = "/volume1/photo/exif_photo_$(date +%Y%m%d_%H%M%S).log" ]; then
    log_file="/volume1/photo/exif_photo_$(date +%Y%m%d_%H%M%S).log"
fi
log_message "Starting EXIF photo resort process at $(date)" true false
if [ "$dry_run" = true ]; then
...
find "$source_dir" -type f -print0 | while IFS= read -r -d '' file; do
    ...
    if [[ "${allowed_extensions[*]}" =~ "$file_extension" ]]; then
        # Check image dimensions for small images
        if check_image_dimensions "$file"; then
            # Image is large enough, proceed with normal sorting
            ...
            if [ "$move_small_files" = true ]; then
                # Image is too small, move to DELETE folder
                delete_path="$delete_dir/$(basename "$file")"
                perform_operation "move" "$file" "$delete_path" "Moved small image $file to $delete_path"
                ((deleted_files++))
                
                # Handle associated thumbnails for deleted files
                handle_thumbnails "$file" "$delete_path"
            else
                log_message "Skipped $file (small image, delete disabled)" false false
                ((skipped_files++))
            fi
        fi
    else
...
log_message "Completed EXIF photo resort process at $(date)" true false
if [ "$dry_run" = true ]; then
